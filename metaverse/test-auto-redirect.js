...
async function testAutoRedirect() {
    console.log('🧪 Testing Auto-Redirect for Duplicate Connections...\n');
    
    const spaceId = 'test-space-auto-redirect';
    ...
    console.log('\n2. Testing second connection (should be blocked and trigger auto-redirect)...');
    ...
    await new Promise((resolve) => {
        ...
        ws2.on('message', (data) => {
            ...
            if (message.type === 'error') {
                console.log('✅ WS2 correctly blocked with error:', message.payload.message);
                if (message.payload.message.includes('already connected')) {
                    console.log('✅ Auto-redirect should be triggered in frontend');
                }
                ws2.close();
                resolve();
    ...
    // Clean up
    ws1.close();
    console.log('\n✅ Auto-redirect test completed');
    console.log('\n📋 Summary:');
    console.log('- First connection: Should succeed');
    console.log('- Second connection (same user): Should be blocked with error');
    console.log('- Third connection (different user): Should succeed');
    console.log('- Frontend should auto-redirect to dashboard for blocked connections');
}
